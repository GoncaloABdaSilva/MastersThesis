...
def load_scores():
    global best_f1_score, best_miou, best_loss, avg_loss_list, f1_list, miou_list, precision_list, recall_list
...
def save_epoch(state, epoch: int, filename="checkpoint_epoch_", extension=".pth.tar"):
    print(f"\n=> Saving checkpoint at epoch {epoch}.")
    torch.save(state, filename + str(epoch) + extension)
...
def check_accuracy(loader, model, checkpoint, loss_fn, device):
    ...
    if device == 0:
        ...
        f1_list.append(curr_f1_score.item())
        miou_list.append(curr_miou.item())
        precision_list.append(curr_precision.item())
        recall_list.append(curr_recall.item())
        save_json_score()
