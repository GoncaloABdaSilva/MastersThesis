...
def binary_focal_loss_split(probs, targets, alpha, gamma):
...
def bce_dice_loss_function(probs, targets, epsilon=1e-7):
    #probs = torch.sigmoid(logits)
    ...
    bce_loss = -(targets_1d * torch.log(probs_1d) + ((1 - targets_1d) * torch.log(1 - probs_1d))).mean()
    ...
    #bce_loss = nn.BCEWithLogitsLoss()(probs, targets)
    '''with torch.amp.autocast(device_type='cuda', enabled=False):
        bce_loss = nn.BCELoss()(probs.float(), targets.float())'''
...
def main(rank: int, world_size: int, max_epochs: int, load_model: bool):
    ...
    #loss_fn = lambda probs, targets: binary_focal_loss_split(probs, targets, alpha=3.0, gamma=1.0)
    loss_fn = lambda probs, targets: bce_dice_loss_function(probs, targets)
