...
PIN_MEMORY = True
#LOAD_MODEL = False
THREECLASS_CRACKTREE260_DS_KAGGLE = "/kaggle/input/threeclasscracktree260/ThreeClassCrackTree260"
TRAIN_IMG_DIR = THREECLASS_CRACKTREE260_DS_KAGGLE + "/train/img"
TRAIN_MASK_DIR = THREECLASS_CRACKTREE260_DS_KAGGLE + "/train/gt"
VAL_IMG_DIR = THREECLASS_CRACKTREE260_DS_KAGGLE + "/val/img"
VAL_MASK_DIR = THREECLASS_CRACKTREE260_DS_KAGGLE + "/val/gt"
...
def train_fn(loader, model, optimizer, loss_fn, scaler, gpu_id):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        targets = targets.long().to(device=gpu_id) 
...
def main(rank: int, world_size: int, max_epochs: int, load_model: bool):
    ...
    model = UNET(in_channels=3, out_channels=3).to(gpu_id)
    ...
    loss_fn = nn.CrossEntropyLoss(weight=weights).to(gpu_id)
